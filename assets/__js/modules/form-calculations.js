...
// Legacy alias / compatibilidad interna
...
// --------- Utilitarios de fecha/antigüedad ---------
...
function calcularRecargos(modalidad, valoresForm) {
	...
	let maximoAcumulableTotal =
		modalidad.acf?.maximo_acumulable_total !== undefined
			? Number(modalidad.acf.maximo_acumulable_total)
			: null;
	if (isNaN(maximoAcumulableTotal)) maximoAcumulableTotal = null;
	...
	for (const sup of suplementos) {
		...
		if (comparador(inputValor, sup.valor)) {
	...
	for (const grupo in grupos) {
		...
		if (arr.length) {
			if (esAcumulable || arr.length === 1) {
				...
				const descripcionesTxt = joinConY(descripciones);
				html += `<li class="form__plan-recargos-item"><span class="form__plan-porcentaje-recargo">Recargo ${Math.round(
					maxRecargo * 100
...
// Escucha cambios en select de vendedor/admin
const usuarioRolInput = document.getElementById("usuario-rol");
if (usuarioRolInput) {
	usuarioRolInput.addEventListener("change", async () => {
		// estado loading precios
		document
...
// Inicialización inmediata para disparar lógica al cargar
setTimeout(async () => {
	if (isProfesional()) {
		await ensureCurrentUserIdReady();
	}
...
function renderPlans(modalidades, valoresForm) {
	...
	const preciosConIVA = getFormState().preciosConIVA;
	...
	plansContainer.innerHTML = sorted
		.map((m, idx) => {
			...
			const breakdown = calcularRecargos(m, {
				...valoresForm,
				fecha_primera_matriculacion: getElValue("fecha_primera_matriculacion"),
				kilometros: getElValue("kilometros"),
				traccion: getElValue("traccion"),
				cambio: getElValue("cambio"),
				doble_motor: getElValue("doble_motor") || null,
			});
			...
			let recargosHTML = "";
			if (isAdmin() && precioBase !== null && precioFinal !== null) {
...
async function filtrarModalidades() {
	...
	const valoresForm = {
		cilindrada: formState.cilindrada || 0,
		potencia: formState.potencia || 0,
		duracion: Number(getElValue("duracion")) || 12,
		traccion_camion: formState.traccionCamion || null,
		mma: formState.mma || null,
	...
	disponibles = disponibles.filter((m) => {
		...
		for (const config of [
			...
			if (condicionesEspecialesArr.includes(config.key)) {
				const valorFormulario = getElValue(config.formField);
				const valoresModalidad = (() => {
					const campo = cm[config.modalidadField];
					if (Array.isArray(campo)) {
						return campo.map((v) => (typeof v === "object" ? v.value : v));
					}
