...
function isPotentiallyValidMatricula(value) {
	if (/[^A-Za-z0-9]/.test(value)) {
		return false;
	}
	...
	for (const { prefix, rest } of prefixOptions) {
		...
		if (!/^\d{4}/.test(firstFour)) continue;
...
export function validateMatriculaField(input, showError, isHardCheck = false) {
	...
	if (!isHardCheck) {
		// Prioridad: caracteres no permitidos (tanto clásicas como modernas)
		if (/[^A-Za-z0-9]/.test(value)) {
			if (showError) setError(input, "Carácter no permitido. Usa solo letras y números");
			return false;
		}

		// Moderna parcial/tiempo real
		if (/^\d/.test(value)) {
		...
		if (!isPotentiallyValidMatricula(value)) {
			...
			if (
				...
				![...PROVINCIAS].some((p) => p.startsWith(first))
			) {
