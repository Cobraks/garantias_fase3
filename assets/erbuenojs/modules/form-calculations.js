...
function getDescuentosAplicables(modalidad, { incluirCaducadas = false } = {}) {
	...
	(window.currentOfertas || []).forEach((oferta) => {
		...
		// Solo aplicar si la oferta realmente aplica a esta modalidad
...
function getUsuarioRolSeleccionado() {
	...
	if (ENABLE_LOGS) {
		log(
			"[getUsuarioRolSeleccionado] role:",
			role,
	...
	// Profesional / go_profesional usan currentUserId
	if (role === "go_profesional" || role === "go_profesional") {
		...
		if (ENABLE_LOGS)
			log(
				"[getUsuarioRolSeleccionado] currentUserId no válido aún:",
				window.currentUserId
			);
	}
	return null;
...
// helper: espera breve a que exista currentUserId si el rol es profesional/go_profesional
async function ensureUserIdReady() {
	const role = getEffectiveUserRole();
	if (!(role === "go_profesional" || role === "go_profesional")) return;
	...
	while (
		...
		if (ENABLE_LOGS)
			log(
				"[ensureUserIdReady] esperando currentUserId, llevamos",
				Date.now() - start,
				"ms"
			);
	...
	if (ENABLE_LOGS)
		log("[ensureUserIdReady] resultado currentUserId:", window.currentUserId);
}

// Debounce de refresco visual de ofertas según modalidades actuales
...
async function refreshOfertasDisplay(attempt = 0) {
	...
	const usuarioId = getUsuarioRolSeleccionado();
	if (!usuarioId) {
		if ((role === "go_profesional" || role === "go_profesional") && attempt < 5) {
			if (ENABLE_LOGS)
				log(
	...
	_refreshOfertasPending = setTimeout(async () => {
		const ofertas = window.currentOfertas || (await fetchOfertas(usuarioId));
		window.currentOfertas = ofertas;
		const modalidadesVisibles = Array.isArray(window.visibleModalidades)
			? window.visibleModalidades
			: [];
...
async function updateOfertas() {
	// Profesional: asegurar currentUserId si necesario
	const role = getEffectiveUserRole();
	if (role === "go_profesional" || role === "go_profesional") {
		await ensureUserIdReady();
	}

	// Obtener el usuario y sus ofertas
	const usuarioId = getUsuarioRolSeleccionado();
	if (!usuarioId) return;
	const ofertas = await fetchOfertas(usuarioId);
	window.currentOfertas = ofertas || [];

	// Modalidades visibles actuales (para filtrar listado)
	const modalidadesVisibles = Array.isArray(window.visibleModalidades)
		? window.visibleModalidades
		: [];

	// Actualiza el listado de ofertas mostrado, filtrando por modalidades visibles
	await updateOfertasList(usuarioId, modalidadesVisibles);

	if (ENABLE_LOGS) log("Ofertas activas usuario:", ofertas);

	// Recalcular planes (porque descuentos cambian)
	if (typeof window.filtrarModalidades === "function") {
		window.filtrarModalidades();
	}

	// Notificar a otros módulos interesados (ej. resumen contratación)
...
setTimeout(async () => {
	// Si es profesional/go_profesional, asegurar que tenemos userId antes de disparar ofertas
	const role = getEffectiveUserRole();
	if (role === "go_profesional" || role === "go_profesional") {
		await ensureUserIdReady();
	}
...
function renderPlans(modalidades, valoresForm) {
	...
	// Detectar preseleccionadas y selección persistida
	const persistedModalidadId = window.FormCache?.selectedModalidadId;
...
async function filtrarModalidades() {
	...
	// Guardar visibles globalmente para que updateOfertas pueda usarlos
	window.visibleModalidades = disponibles;
	...
	// Refresca ofertas visibles tras aplicar filtros nuevos
	refreshOfertasDisplay();

	return disponibles; // útil si alguien espera el array (como updateOfertas)
}
...
async function initCalculations() {
	...
	if (inputVendedor) {
		inputVendedor.addEventListener("change", async () => {
			// Mostrar shimmer / limpiar valores previos
			document
			...
			// Primero actualiza ofertas y luego recalcula modalidades
			await updateOfertas();
		});
	}
	...
	// asegurar que currentUserId está disponible si hace falta para profesional
	const role = getEffectiveUserRole();
	if (role === "go_profesional" || role === "go_profesional") {
		await ensureUserIdReady();
	}
	// primero filtremos modalidades y luego forcemos actualización de ofertas (importante para profesionales)
	await filtrarModalidades();
	if (
		!document.getElementById("usuario-rol") &&
		(role === "go_profesional" || role === "go_profesional")
	) {
		await updateOfertas();
	}
}
