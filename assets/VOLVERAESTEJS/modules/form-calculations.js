	...
	html += `</ul>`;
        ...
        if (descuentosAplicadosSync.length) {
                ...
                for (const desc of descuentosAplicados) {
                        html += `<li class="form__plan-descuentos-item"><span class="form__plan-porcentaje-descuento">-${Math.round(desc.porcentaje * 100)}%:</span> ${desc.nombre}</li>`;
                }
                html += `</ul>`;
        }
...
async function renderPlans(modalidades, valoresForm) {
	...
	plansContainer.innerHTML = sorted
        ...
        const persistedModalidadId = FormCache?.selectedModalidadId;
        const anySelectedPre = sorted.some((m) => {
                const estilos =
                        m.acf?.condiciones_generales_y_tarifas?.descripcion_informacion
                                ?.estilos || {};
        ...
        const planHtmls =  Promise.all(
                sorted.map(async (m, idx) => {
                        ...
                        const descuentos = await getDescuentosAplicables(m);
                        let multiplicador = 1;
                        descuentos.forEach((d) => {
                                multiplicador *= 1 - d.porcentaje;
                        });
                        ...
                        const precioSinDescuentoIVA =
                                precioSinDescuento !== null
                                        ? redondearEuros(
                                                precioSinDescuento * (1 + IVA_PORCENTAJE / 100)
                                        )
                                        : null;
                        ...
                        const precioIVA =
                                precioFinal !== null
                                        ? redondearEuros(
                                                precioFinal * (1 + IVA_PORCENTAJE / 100)
                                        )
                                        : null;
                        const ivaSolo =
                        ...
                        return `
            <div class="${planClasses.join(" ")}" tabindex="0" role="button" data-plan-idx="${idx}" data-modalidad-id="${m.ID}">
                ${badge}
                <div class="form__plan-content">
                    ...
                    <div class="form__plan-price-wrapper">
                        ...
                        <span class="form__plan-price-text">
                            <span class="plan-price-value" style="display:${preciosConIVA ? "inline" : "none"}">${priceConIva}</span>
                            <span class="plan-price-value-noiva" style="display:${preciosConIVA ? "none" : "inline"}">${priceSinIva}</span>
                            <span class="form__plan-price-euro">€</span>
                        </span>
                    </div>
                    <div class="form__plan-iva" style="display:${preciosConIVA ? "block" : "none"}">IVA incluido</div>
                    <div class="form__plan-iva-no" style="display:${preciosConIVA ? "none" : "block"}">${priceIvaOnly}</div>
        ...
        plansContainer.innerHTML = planHtmls.join("");
...
async function filtrarModalidades() {
	...
	if (!disponibles.length && !garantiaNoDisponiblePorAntiguedad) {
                ...
                await renderPlans([], valoresForm);
	} else if (disponibles.length) {
                updateDuracionSelect(mesesDisponibles);
                await renderPlans(disponibles, valoresForm);
	} else {
                updateDuracionSelect([]);
                await renderPlans([], valoresForm);
	}
