...
function getSummaryText(fieldId) {
	...
	if (withIvaEl && window.getComputedStyle(withIvaEl).display !== "none") {
		price = withIvaEl.textContent.trim();
	} else if (noIvaEl && window.getComputedStyle(noIvaEl).display !== "none") {
...
function setupSummaryButtons() {
	document.querySelectorAll(".summary-button").forEach((btn) => {
		btn.addEventListener("click", () => {
			...
			if (
				typeof window.isCurrentTabValid === "function" &&
				targetTabIndex > window.FormCache.currentTab &&
				!window.isCurrentTabValid()
			) {
				alert("Completa todos los campos antes de continuar.");
				return;
			}
			if (typeof window.FormCache !== "undefined") {
				window.FormCache.currentTab = targetTabIndex;
			}
			if (typeof window.showTab === "function") {
				window.showTab(targetTabIndex);
			}
		});
	});
}
...
export default function initSummary() {
	...
	setupSummaryButtons();
	setupSummaryRefresh();
	// Expón helpers globales
	window.updateSummary = updateSummary;
	window.debouncedUpdateSummary = debouncedUpdateSummary;
	window.showSummarySectionForTab = showSummarySectionForTab;
}
