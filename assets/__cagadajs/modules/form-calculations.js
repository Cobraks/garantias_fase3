...
async function ensureOfertasLoaded() {
	...
	if (!hasProf) return;
	if (!Array.isArray(window.currentOfertas)) {
		await ensureCurrentUserIdReady();
		const uid = getEffectiveProfessionalId();
		if (uid) {
			window.currentOfertas = await fetchOfertas(uid);
		}
	}
}
...
async function getDescuentosAplicables(
	...
	if (!Array.isArray(window.currentOfertas)) return [];
...
function renderRecargosHTML({
	...
	const descuentosAplicadosSync = window.currentOfertas
...
function renderPlans(modalidades, valoresForm) {
	...
	plansContainer.innerHTML = sorted
		.map((m, idx) => {
			...
			// descuentos total (sin await por performance; se usa solo para mostrar porcentaje)
			const descuentoTotalSync = 0; // se mantiene como 0 aquí porque el cálculo endurecido está separado
...
Podrías encapsular window.currentOfertas en un módulo propio y evitar el global, pero eso puede venir en una fase siguiente.*/