...
/**
 * Normaliza campo aplicación de la oferta
 */
...
export async function fetchOfertas(userId, { force = false } = {}) {
	// Si no hay userId, esperamos a que el profesional (si aplica) esté listo
	if (!userId && isProfesional()) {
...
// Refresco inteligente de ofertas basado en profesional + modalidades visibles
...
export async function refreshOfertasDisplay(attempt = 0) {
	...
	_refreshOfertasPending = setTimeout(async () => {
		...
		const existing = getCurrentOfertas();
		const ofertas =
			existing && Array.isArray(existing)
				? existing
				: await fetchOfertas(effectiveUserId);
		setCurrentOfertas(ofertas);
...
// Legacy mínima: alias para compatibilidad temporal
