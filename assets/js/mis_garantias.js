...
(() => {
   ...
   function initResizableColumns(table) {
    ...
    const inlineCountdownValue =
        coverageCountdownValue && coverageCountdownValue !== "—"
            ? `${coverageCountdownValue} ${
                    coverageCountdownLabel === "Días para inicio" ? "para inicio" : "restantes"
                }`
            : "";

    const inlineCountdownHtml = inlineCountdownValue
        ? `<div class="detail__inline-meta">` +
              `${coverageCountdownLabel ? `<strong>${coverageCountdownLabel}</strong>` : ""}` +
