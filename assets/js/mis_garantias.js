...
(() => {
    ...
    const professionalValidationHtml = escapeHtml(
        "Has indicado que has realizado la transferencia. El equipo de 360VO revisará la información y activará tu garantía en un plazo máximo de 72 horas. Recibirás un correo cuando esté activa."
    );
    const adminValidationHtml = vendorDisplayHtml
        ? `${vendorDisplayHtml} ha indicado que ha realizado la transferencia. Revisa la operación y activa la garantía cuando proceda.`
        : escapeHtml(
              "El cliente ha indicado que ha realizado la transferencia. Revisa la operación y activa la garantía cuando proceda."
          );
        ...
        if ((isPendientePago || isValidacionPendiente) && metodoPago === "transferencia") {
            ...
            const noteHtml = isValidacionPendiente
                ? `<p class="detail__payment-note detail__payment-note--validation">${
                      isAdmin ? adminValidationHtml : professionalValidationHtml
                  }</p>`
                : isAdmin
                ? adminNoteHtml
