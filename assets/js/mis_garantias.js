...
(() => {
		...
		let lastValidResults = [];
                ...
                function setMobileDetailOpen(
                        open,
                        { focus = true, restoreFocus = true, preserveSummary = false } = {}
                ) {
                        if (!detail) {
                                return;
                        }
   ...
   function initResizableColumns(table) {
           ...
           const enableResizableColumns = () => {
                   if (!teardown) {
                           teardown = setup();
                   }
           };

           const disableResizableColumns = () => {
                   if (teardown) {
                           teardown();
                           teardown = null;
                   }
           ...
           if (!desktopOnlyQuery || desktopOnlyQuery.matches) {
           ...
           if (typeof desktopOnlyQuery.addEventListener === "function") {
                   desktopOnlyQuery.addEventListener("change", handleDesktopToggle);
           } else if (typeof desktopOnlyQuery.addListener === "function") {
                   desktopOnlyQuery.addListener(handleDesktopToggle);
           }
   }
