    ...
    const formattedAmount = formatAmountForMessage(priceRaw);
        ...
        tr.dataset.compradorTelefono = cliente_telefono_dataset;
                ...
                function setAdminSummaryLoading(root, isLoading) {
                        ...
                        if (caption) {
                                caption.textContent = loading
                                        ? ADMIN_SUMMARY_LOADING_MESSAGE
                                        : caption.textContent;
                        }
                        const updated = root.querySelector("[data-admin-summary-updated]");
                        if (updated) {
                                updated.textContent = loading
                                        ? ADMIN_SUMMARY_CARD_LOADING
                                        : updated.textContent;
                        }
                        ...
                        const refresh = root.querySelector("[data-admin-summary-refresh]");
                        if (refresh) {
                                refresh.hidden = loading;
                ...
                function renderAdminSummary(root, data = {}) {
                        ...
                        if (totalEl) {
                                totalEl.textContent = formatIntegerValue(totals.count || 0);
                        }

                        const updatedLabel = formatUpdatedLabel(data.updated_at || "");
                        const caption = root.querySelector("[data-admin-summary-caption]");
                        if (caption) {
                                caption.textContent = updatedLabel
                        ...
                        if (statesList) {
                                ...
                                if (states.length === 0) {
                                        ...
                                        emptyItem.className = "guarantee-admin-summary__state-item is-empty";
                                        emptyItem.textContent = "Sin datos disponibles";
                                        statesList.appendChild(emptyItem);
                                } else {
                                        const maxCount = states.reduce(
                                                (acc, state) => {
                                                        const count = normalizeToInt(state.count || 0);
                                        ...
                                        states.forEach((state) => {
                                                ...
                                                li.className = "guarantee-admin-summary__state-item";
                                                if (state.value) {
                                                        li.dataset.state = state.value;
                                                }
                                                const count = normalizeToInt(state.count || 0);
                                                const progress = maxCount > 0 ? Math.round((count / maxCount) * 100) : 0;
                                                li.innerHTML = `
                                                        <div class="guarantee-admin-summary__state-row">
                                                                <span class="guarantee-admin-summary__state-label">
                                                                        <span class="guarantee-admin-summary__state-dot" aria-hidden="true"></span>
                                                                        ${escapeHtml(state.label || state.value || "")}
                                                                </span>
                                                                <span class="guarantee-admin-summary__state-count">${formatIntegerValue(
                                                                        count
                                                                )}</span>
                                                        </div>
                                                        <div class="guarantee-admin-summary__state-bar" role="presentation">
                        ...
                        if (pendingList) {
                                ...
                                const pending = data && typeof data === "object" ? data.pending || {} : {};
                                const metrics = [
                                        {
                                                key: "collect",
                                ...
                                const amounts = metrics.map((metric) => {
                                        const entry = pending[metric.key] || {};
                                        return normalizeToFloat(entry.amount || 0);
                                });
                                const maxAmount = amounts.reduce((acc, amount) => (amount > acc ? amount : acc), 0);
                                metrics.forEach((metric, index) => {
                                        const entry = pending[metric.key] || {};
                                        const amount = normalizeToFloat(entry.amount || 0);
                                        const count = normalizeToInt(entry.count || 0);
                                        const li = document.createElement("li");
                                        li.className = "guarantee-admin-summary__pill";
                                        li.dataset.metric = metric.metric;
                                        const progress = maxAmount > 0 ? Math.round((amount / maxAmount) * 100) : 0;
                                        li.innerHTML = `
                                                <span class="guarantee-admin-summary__pill-label">${escapeHtml(
                                                        metric.label
                                                )}</span>
                                                <strong class="guarantee-admin-summary__pill-amount">${formatCurrencyValue(
                                                        amount
                                                )}</strong>
                                                <span class="guarantee-admin-summary__pill-count">${formatGuaranteeCount(
                                                        count
                                                )}</span>
                                                <div class="guarantee-admin-summary__pill-bar" role="presentation">
                                                        <span style="width: ${progress}%"></span>
                                                </div>
                                        `;
                        ...
                        const monthLabel = root.querySelector("[data-admin-summary-month-label]");
                        const monthCount = root.querySelector("[data-admin-summary-month-count]");
                        const monthAmount = root.querySelector("[data-admin-summary-month-amount]");
                        const monthTop = root.querySelector("[data-admin-summary-month-top]");
                        const monthData = data && typeof data === "object" ? data.month || {} : {};

                        if (monthLabel) {
                                monthLabel.textContent = monthData.label || "—";
                        }
                        if (monthCount) {
                                monthCount.textContent = formatGuaranteeCount(monthData.count || 0);
                        }
                        if (monthAmount) {
                                monthAmount.textContent = formatCurrencyValue(monthData.amount || 0);
                        }
                        if (monthTop) {
                                const topState = monthData.top_state || null;
                                if (topState && (topState.label || topState.value)) {
                                        ...
                                        monthTop.textContent = `${label} · ${formatGuaranteeCount(topCount)}`;
                                } else {
                ...
                function bindAdminSummaryRefresh(root) {
                        const button = root ? root.querySelector("[data-admin-summary-refresh]") : null;
                        ...
                        button.addEventListener("click", (event) => {
                                ...
                                loadAdminSummary(root, { force: true });
                        });
                }
                ...
                function loadAdminSummary(root, options = {}) {
                        ...
                        fetchAdminSummary(force)
                                .then((payload) => {
                                        ...
                                        setAdminSummaryLoading(root, false);
                                })
                                .catch((error) => {
                                        console.error(error);
                                        root.dataset.loaded = "";
                ...
                function renderEmptyDetail(mode = "awaiting") {
                        ...
                        return `
                                <div class="guarantee-detail__empty" data-empty-detail data-empty-mode="awaiting">
                                        ...
                                        <p>Haz clic en una fila para consultar la información completa en este panel.</p>
                                </div>
                        `;
                }
