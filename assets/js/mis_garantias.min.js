...
(() => {
   ...
   function initResizableColumns(table) {
    ...
    const hasBuyerInfo = buyerFields.every((field) => isFilled(pickField(field, "")));
      ...
      const formatTimelineDate = (raw) => {const fallback=()=>{if(typeof raw==="string"){const trimmedRaw=raw.trim();if(trimmedRaw!==""){return trimmedRaw}}if(raw!==undefined&&raw!==null){const stringified=String(raw).trim();if(stringified!==""){return stringified}}return "—"};const parsed=parseTimelineDate(raw);if(!parsed){return fallback()}try{const formatter=new Intl.DateTimeFormat("es-ES",{day:"2-digit",month:"short",year:"numeric"});const parts=formatter.formatToParts(parsed);const day=parts.find((part)=>part.type==="day")?.value??"";const month=parts.find((part)=>part.type==="month")?.value??"";const year=parts.find((part)=>part.type==="year")?.value??"";if(day&&month&&year){const normalizedMonth=month.replace(/\.$/,"").toLowerCase();return `${day} ${normalizedMonth}, ${year}`}return formatter.format(parsed)}catch(error){return fallback()}};
