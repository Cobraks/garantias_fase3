(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleLookup = async () => {
      ...
      if (email === '') {
        showGlobal('No hemos podido recuperar tu correo electrónico. Regresa al inicio de sesión e inténtalo de nuevo.', 'error');
    ...
    const handleVerify = async () => {
      if (!state.token) {
        showGlobal('No hemos encontrado ninguna solicitud pendiente. Solicita un nuevo código desde el inicio de sesión.', 'error');
        return;
      }
      if (!codeField) return;
