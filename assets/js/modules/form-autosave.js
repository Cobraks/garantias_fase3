...
function mergePresentationFlags(globalFlags = {}, userFlags = {}) {
        const merged = { ...DEFAULT_PROFORMA_PRESENTATION, ...globalFlags };
        Object.keys(DEFAULT_PROFORMA_PRESENTATION).forEach((key) => {
                if (typeof userFlags[key] === "boolean") {
                        merged[key] = userFlags[key];
                }
        });
        return merged;
}
...
function normalizeProformaUserConfig(acf = {}) {
        const facturacion = acf.facturacion || {};
        const proforma = facturacion.proforma || {};
        return {
                enabled: proforma.activar_proforma,
                presentation: normalizePresentationFlags(proforma.presentacion_notificaciones || {}),
        };
}
...
async function resolveProformaEligibility({
        ...
        defaultResult.transferIban = normalizeText(globalConfig.transferIban || "");

        if (!globalConfig.enabled) {
                return {
                        allowed: false,
                        presentation: basePresentation,
                        normalizedPaymentMethod: defaultResult.normalizedPaymentMethod,
                        transferIban: defaultResult.transferIban,
                };
        }

        if (globalConfig.roles.length > 0 && !globalConfig.roles.includes(normalizedRole)) {
                return {
                        allowed: false,
                        presentation: basePresentation,
                        normalizedPaymentMethod: defaultResult.normalizedPaymentMethod,
        ...
        if (vendorUserId) {
                try {
                        ...
                        userConfig = normalizeProformaUserConfig(acf || {});
                } catch (err) {
                        console.warn("[AUTOSAVE] resolveProformaEligibility user error", { vendorUserId, err });
                }
        }

        const mergedPresentation = mergePresentationFlags(basePresentation, userConfig?.presentation || {});
        ...
        let allowed = true;
        if (normalizedRole === "profesional") {
                const allowedModes = globalConfig.showForProfessionals;
                if (allowedModes.length > 0) {
                        allowed = allowedModes.includes(paymentPref.normalizedMethod);
                }
        }
        ...
        return {
                allowed,
                presentation: mergedPresentation,
                normalizedPaymentMethod: paymentPref.normalizedMethod || defaultResult.normalizedPaymentMethod,
                transferIban: defaultResult.transferIban,
        };
}
