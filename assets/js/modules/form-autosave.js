...
export default function initAutosave() {
        ...
        function computeProformaSignature(rawArgs = {}) {
                ...
                const payload = { templateUrl, items, coverageLabel, matricula, emissionDate, dueDate };
		...
		const importeRect = resolveRect("importe_1");
                ...
                if (refEmisionRect) {
                        ...
                        const baseY = refEmisionRect.y + (referenceLineHeight - referenceFontSize) / 2;
                        const startY = baseY + (referenceLines.length - 1) * referenceLineHeight;

                        referenceLines.forEach((line, index) => {
                                const y = startY - index * referenceLineHeight;
                                const color =
