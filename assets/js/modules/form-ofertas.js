...
const SPECIAL_FIXED_LABEL = "Tarifa Fija";
const SPECIAL_MONTHS_SUFFIX = "m";
...
function formatDurationLabel(special) {
        if (special && typeof special.duracion_label === "string" && special.duracion_label.trim() !== "") {
                return special.duracion_label.trim().replace(/\s*mes(?:es)?\b/gi, " m").trim();
        }
        const months = toPositiveInt(special?.duracion_meses);
        if (months !== null) {
...
function getSelectedDurationMonths() {
...
function collectSpecialFixedMatches(modalidades, { duration = null } = {}) {
        ...
        const targetDuration = toPositiveInt(duration);
        ...
        mods.forEach((modalidad) => {
                specials.forEach((special) => {
                        ...
                        const offerDuration = toPositiveInt(special?.duracion_meses);
                        if (targetDuration !== null && offerDuration !== null && offerDuration !== targetDuration) {
                                return;
                        }
                        const key = [
...
export async function updateOfertasList(
        ...
        visibles.forEach((oferta) => {
                ...
                } else if (esIvaIncluido) {
                        li.textContent = oferta.nombre || oferta.etiqueta || "IVA incluido";
                } else {
