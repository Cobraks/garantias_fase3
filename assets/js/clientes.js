(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function initializeClientsSummary(root = document) {
            ...
            const panelsContainer = summaryRoot.querySelector('[data-clients-summary-panels]');
            const panels = summaryRoot.querySelectorAll('[data-clients-summary-panel]');
            const toggles = summaryRoot.querySelectorAll('[data-clients-summary-toggle]');
            ...
            const updatePanelHeight = () => {
                const activePanel = panelsContainer.querySelector('.is-active');
                if (!activePanel) {
                    return;
                }
                const { height } = activePanel.getBoundingClientRect();
                panelsContainer.style.setProperty('--clients-summary-panel-height', `${height}px`);
            };

            const setContext = (value) => {
                const contextValue = value === 'month' ? 'month' : 'year';
            ...
            toggles.forEach((toggle) => {
                toggle.addEventListener('change', () => {
                    if (toggle.checked) {
                        setContext(toggle.value);
                    }
                });
            });

            const defaultToggle = summaryRoot.querySelector('[data-clients-summary-toggle]:checked');
            setContext(defaultToggle ? defaultToggle.value : (summaryRoot.dataset.context || 'year'));
        ...
        function renderAvatar(profile, altText, initialsSource = '') {
            ...
            const palette = pickAvatarPalette(fallbackSeed);
            const styleAttr = palette
                ? ` style="--avatar-bg:${palette.bg};--avatar-color:${palette.fg};--avatar-border:${palette.border};"`
                : '';

            if (avatarUrl !== '') {
                return `<img src="${escapeAttribute(avatarUrl)}" alt="${escapeAttribute(altText)}" class="clients-table__avatar"${styleAttr}>`;
            }

            const initialsFromProfile = typeof safeProfile.initials === 'string' ? safeProfile.initials.trim() : '';
            const computedInitials =
                buildInitialsFromName(initialsSeed) ||
                buildInitialsFromName(initialsFromProfile) ||
                buildInitialsFromName(altText);
            const safeInitials = computedInitials || '--';

            return `<span class="clients-table__initials"${styleAttr}>${escapeHtml(safeInitials)}</span>`;
        }
        ...
        function createOffersDialog(options = {}) {
            ...
            let specialChoices = { tipo_de_garantia: [], nivel_garantia: [], duracion_maxima: [] };
            let modalities = [];
            let isLoading = false;
            let isSaving = false;
            let dirty = false;
            ...
            const specialChoiceMaps = {
                tipo: new Map(),
                nivel: new Map(),
                duration: new Map(),
            };

            let addOfferPopover = null;
            let addOfferPopoverCleanup = null;

            function resetState() {
                closeAddOfferPopover();
                offers = [];
                choices = { tipo_oferta: [], aplicacion: [] };
                specialChoices = { tipo_de_garantia: [], nivel_garantia: [], duracion_maxima: [] };
                modalities = [];
                isLoading = false;
                isSaving = false;
                dirty = false;
                ...
                specialChoiceMaps.tipo.clear();
                specialChoiceMaps.nivel.clear();
                specialChoiceMaps.duration.clear();
                if (body) {
                    body.innerHTML = '';
                }
            }
            ...
            function appendAndFocusOffer(builder) {
                const newOffer = builder();
            ...
            function addStandardOffer() {
                appendAndFocusOffer(() => createEmptyOffer());
            }

            function addFixedPriceOffer() {
                appendAndFocusOffer(() => createEmptyFixedOffer());
            }

            function closeAddOfferPopover() {
                if (typeof addOfferPopoverCleanup === 'function') {
                    addOfferPopoverCleanup();
                    addOfferPopoverCleanup = null;
                }
                if (addOfferPopover && addOfferPopover.parentNode) {
                    addOfferPopover.parentNode.removeChild(addOfferPopover);
                }
                addOfferPopover = null;
            }

            function positionAddOfferPopover(popover) {
                if (!addButton || !popover) {
                    return;
                }
                ...
                const preferredTop = buttonRect.top + window.scrollY - popRect.height - spacing;
                const fallbackTop = buttonRect.bottom + window.scrollY + spacing;
                const top = preferredTop >= window.scrollY + 8 ? preferredTop : fallbackTop;

                let left = buttonRect.left + window.scrollX + (buttonRect.width - popRect.width) / 2;
            ...
            function openAddOfferPopover() {
                ...
                popover.className = 'client-offers__add-popover';
                popover.setAttribute('role', 'menu');

                const options = document.createElement('div');
                options.className = 'client-offers__add-options';

                const standardButton = document.createElement('button');
                standardButton.type = 'button';
                standardButton.className = 'client-offers__add-option';
                standardButton.textContent = strings.manageOffersAddPercent || 'Porcentaje descuento / sin suplementos';
                standardButton.setAttribute('role', 'menuitem');
                ...
                fixedButton.className = 'client-offers__add-option client-offers__add-option--primary';
                fixedButton.textContent = strings.manageOffersAddFixed || 'Precio fijo';
                fixedButton.setAttribute('role', 'menuitem');
                fixedButton.addEventListener('click', () => {
                    closeAddOfferPopover();
                    addFixedPriceOffer();
                });

                options.appendChild(standardButton);
                options.appendChild(fixedButton);
                popover.appendChild(options);

                document.body.appendChild(popover);

                const reposition = () => positionAddOfferPopover(popover);
                window.requestAnimationFrame(() => {
                    reposition();
                    standardButton.focus({ preventScroll: true });
                });

                const handleOutsideInteraction = (event) => {
                    const target = event.target;
                    if (popover.contains(target) || (addButton && addButton.contains(target))) {
                        return;
                    }
                    closeAddOfferPopover();
                };
            ...
            function isFixedOffer(offer) {
                return Boolean(offer && (offer.kind === 'fixed' || offer.special_type === 'fixed_price'));
            }
            ...
            function normalizeSpecialChoices(rawChoices) {
                const normalized = {
                    tipo_de_garantia: [],
                    nivel_garantia: [],
                    duracion_maxima: [],
                ...
                if (rawChoices && typeof rawChoices === 'object') {
                    if (Array.isArray(rawChoices.tipo_de_garantia)) {
                        normalized.tipo_de_garantia = rawChoices.tipo_de_garantia.filter((choice) => choice && choice.value);
                    }
                    if (Array.isArray(rawChoices.nivel_garantia)) {
                        normalized.nivel_garantia = rawChoices.nivel_garantia.filter((choice) => choice && choice.value);
                    }
                    if (Array.isArray(rawChoices.duracion_maxima)) {
                        normalized.duracion_maxima = rawChoices.duracion_maxima.filter((choice) => choice && choice.value);
                    }
                }
                ...
                normalized.tipo_de_garantia.forEach((choice) => {
                    const value = Number(choice.value);
                    if (Number.isFinite(value) && value > 0) {
                        const label = typeof choice.label === 'string' ? choice.label : '';
                        specialChoiceMaps.tipo.set(value, label);
                    }
                });

                normalized.nivel_garantia.forEach((choice) => {
                    const value = Number(choice.value);
                    if (Number.isFinite(value) && value > 0) {
                        const label = typeof choice.label === 'string' ? choice.label : '';
                        specialChoiceMaps.nivel.set(value, label);
                    }
                });

                normalized.duracion_maxima.forEach((choice) => {
                    const value = Number(choice.value);
                    if (Number.isFinite(value) && value > 0) {
                        const label = typeof choice.label === 'string' ? choice.label : '';
                        specialChoiceMaps.duration.set(value, label);
                    }
                });
            ...
            function normalizeStandardOffers(rawOffers) {
            ...
            function normalizeFixedOffers(rawOffers) {
                if (!Array.isArray(rawOffers)) {
                    return [];
                }
                ...
                return rawOffers
                    .map((raw) => {
                        const tipoRaw = raw && typeof raw === 'object' ? raw.tipo_de_garantia : null;
                        const nivelRaw = raw && typeof raw === 'object' ? raw.nivel_garantia : null;
                        const duracionRaw = raw && typeof raw === 'object' ? raw.duracion_maxima : null;
                        const tipoValue = typeof tipoRaw === 'object'
                            ? Number(tipoRaw.value)
                            : Number(tipoRaw);
                        const nivelValue = typeof nivelRaw === 'object'
                            ? Number(nivelRaw.value)
                            : Number(nivelRaw);
                        const duracionValue = typeof duracionRaw === 'object'
                            ? Number(duracionRaw.value)
                            : Number(duracionRaw);

                        const durationLabel = typeof duracionRaw?.label === 'string'
                            ? duracionRaw.label
                            : (specialChoiceMaps.duration.get(duracionValue)
                                || (Number.isFinite(duracionValue) && duracionValue > 0
                                    ? `${duracionValue} ${monthsSuffix}`
                                    : ''));
                        ...
                        return {
                            ...
                            tipo_de_garantia: {
                                value: Number.isFinite(tipoValue) ? tipoValue : 0,
                                label: typeof tipoRaw?.label === 'string'
                                    ? tipoRaw.label
                                    : (specialChoiceMaps.tipo.get(tipoValue) || ''),
                                slug: typeof tipoRaw?.slug === 'string' ? tipoRaw.slug : '',
                            },
                            nivel_garantia: {
                                value: Number.isFinite(nivelValue) ? nivelValue : 0,
                                label: typeof nivelRaw?.label === 'string'
                                    ? nivelRaw.label
                                    : (specialChoiceMaps.nivel.get(nivelValue) || ''),
                                slug: typeof nivelRaw?.slug === 'string' ? nivelRaw.slug : '',
                            },
                            precio_fijo: typeof raw?.precio_fijo === 'number' ? raw.precio_fijo : raw?.precio_fijo || '',
                            excluir_resto_de_niveles: raw?.excluir_resto_de_niveles === true,
                            duracion_maxima: {
                                value: Number.isFinite(duracionValue) && duracionValue > 0 ? duracionValue : 12,
                                label: durationLabel,
                            },
                            estado: raw?.estado !== false,
                            errors: {},
                    ...
                    .filter((offer) => Number.isFinite(offer.tipo_de_garantia.value) && offer.tipo_de_garantia.value > 0
                        && Number.isFinite(offer.nivel_garantia.value) && offer.nivel_garantia.value > 0);
            }

            function normalizeAllOffers(rawStandard, rawFixed) {
                const standard = normalizeStandardOffers(rawStandard || []);
                const fixed = normalizeFixedOffers(rawFixed || []);
            ...
            function createEmptyFixedOffer() {
                const defaultTipo = specialChoices.tipo_de_garantia.find((choice) => Number.isFinite(Number(choice.value)));
                const defaultNivel = specialChoices.nivel_garantia.find((choice) => Number.isFinite(Number(choice.value)));
                const defaultDuration = specialChoices.duracion_maxima.find((choice) => Number.isFinite(Number(choice.value)));
                ...
                return {
                    ...
                    special_type: 'fixed_price',
                    tipo_de_garantia: {
                        value: defaultTipo ? Number(defaultTipo.value) : 0,
                        label: defaultTipo && typeof defaultTipo.label === 'string' ? defaultTipo.label : '',
                        slug: defaultTipo && typeof defaultTipo.slug === 'string' ? defaultTipo.slug : '',
                    },
                    nivel_garantia: {
                        value: defaultNivel ? Number(defaultNivel.value) : 0,
                        label: defaultNivel && typeof defaultNivel.label === 'string' ? defaultNivel.label : '',
                        slug: defaultNivel && typeof defaultNivel.slug === 'string' ? defaultNivel.slug : '',
                    },
                    precio_fijo: '',
                    excluir_resto_de_niveles: false,
                    duracion_maxima: {
                        value: defaultDuration ? Number(defaultDuration.value) : 12,
                        label: defaultDuration && typeof defaultDuration.label === 'string'
            ...
            function buildFixedOfferCard(offer, index) {
                const card = document.createElement('article');
                ...
                const warrantyTypeId = uniqueId('offer-warranty-type');
                const warrantySelect = document.createElement('select');
                warrantySelect.id = warrantyTypeId;
                warrantySelect.className = 'client-offer-card__select';
                const warrantyPlaceholder = document.createElement('option');
                warrantyPlaceholder.value = '';
                warrantyPlaceholder.textContent = strings.manageOffersFixedTypePlaceholder || 'Selecciona un ámbito…';
                warrantySelect.appendChild(warrantyPlaceholder);
                specialChoices.tipo_de_garantia.forEach((choice) => {
                    if (!choice || typeof choice.value === 'undefined') {
                        return;
                    }
                    const option = document.createElement('option');
                    option.value = String(choice.value);
                    option.textContent = typeof choice.label === 'string' ? choice.label : String(choice.value);
                    warrantySelect.appendChild(option);
                });
                warrantySelect.value = offer?.tipo_de_garantia?.value ? String(offer.tipo_de_garantia.value) : '';
                const warrantyFieldObj = createFieldElement(
                    warrantyTypeId,
                    strings.manageOffersFixedTypeLabel || 'Tipo de garantía',
                    warrantySelect,
                );
                grid.appendChild(warrantyFieldObj.field);

                const levelId = uniqueId('offer-level');
                const levelSelect = document.createElement('select');
                levelSelect.id = levelId;
                levelSelect.className = 'client-offer-card__select';
                const levelPlaceholder = document.createElement('option');
                levelPlaceholder.value = '';
                levelPlaceholder.textContent = strings.manageOffersFixedLevelPlaceholder || 'Selecciona una cobertura…';
                levelSelect.appendChild(levelPlaceholder);
                specialChoices.nivel_garantia.forEach((choice) => {
                    if (!choice || typeof choice.value === 'undefined') {
                        return;
                    }
                    const option = document.createElement('option');
                ...
                levelSelect.value = offer?.nivel_garantia?.value ? String(offer.nivel_garantia.value) : '';
                const levelFieldObj = createFieldElement(
                    levelId,
                    strings.manageOffersFixedLevelLabel || 'Cobertura',
                    levelSelect,
                );
                grid.appendChild(levelFieldObj.field);

                const priceId = uniqueId('offer-fixed-price');
                const priceInput = document.createElement('input');
                priceInput.type = 'number';
                priceInput.min = '0';
                priceInput.step = '0.01';
                priceInput.id = priceId;
                priceInput.className = 'client-offer-card__input';
                priceInput.placeholder = strings.manageOffersFixedPricePlaceholder || '';
                priceInput.value = offer.precio_fijo || '';
                const priceFieldObj = createFieldElement(
                    priceId,
                    strings.manageOffersFixedPriceLabel || 'Precio fijo',
                    priceInput,
                );
                grid.appendChild(priceFieldObj.field);

                const durationId = uniqueId('offer-duration');
                const durationSelect = document.createElement('select');
                durationSelect.id = durationId;
                durationSelect.className = 'client-offer-card__select';
                const durationPlaceholder = document.createElement('option');
                durationPlaceholder.value = '';
                durationPlaceholder.textContent = strings.manageOffersFixedDurationPlaceholder || 'Selecciona una duración…';
                durationSelect.appendChild(durationPlaceholder);
                specialChoices.duracion_maxima.forEach((choice) => {
                    if (!choice || typeof choice.value === 'undefined') {
                        return;
                    }
                    const option = document.createElement('option');
                ...
                durationSelect.value = offer?.duracion_maxima?.value ? String(offer.duracion_maxima.value) : '';
                const durationFieldObj = createFieldElement(
                    durationId,
                    strings.manageOffersFixedDurationLabel || 'Duración máxima',
                    durationSelect,
                );
                grid.appendChild(durationFieldObj.field);

                const excludeId = uniqueId('offer-exclude');
                const excludeToggle = document.createElement('label');
                excludeToggle.className = 'client-toggle client-offer-card__toggle';
                excludeToggle.id = excludeId;
                const excludeInput = document.createElement('input');
                excludeInput.type = 'checkbox';
                excludeInput.className = 'client-toggle__input';
                excludeInput.checked = Boolean(offer.excluir_resto_de_niveles);
                const excludeTrack = document.createElement('span');
                excludeTrack.className = 'client-toggle__track';
                const excludeText = document.createElement('span');
                excludeText.className = 'client-toggle__text';
                excludeText.textContent = strings.manageOffersFixedExcludeLabel || 'Excluir resto de niveles';
                excludeToggle.appendChild(excludeInput);
                excludeToggle.appendChild(excludeTrack);
                excludeToggle.appendChild(excludeText);

                const excludeField = document.createElement('div');
                excludeField.className = 'client-offer-card__field';
                const excludeLabel = document.createElement('label');
                excludeLabel.className = 'client-offer-card__label';
                excludeLabel.setAttribute('for', excludeId);
                excludeLabel.textContent = strings.manageOffersFixedExcludeTitle || 'Coberturas disponibles';
                const excludeControl = document.createElement('div');
                excludeControl.className = 'client-offer-card__control';
                excludeControl.appendChild(excludeToggle);
                const excludeNote = document.createElement('p');
                excludeNote.className = 'client-offer-card__note';
                excludeNote.textContent = strings.manageOffersFixedExcludeNote || 'Activado: Solo muestra la cobertura seleccionada. Desactivado: Permite seleccionar el resto de coberturas';
                excludeControl.appendChild(excludeNote);
                const excludeError = document.createElement('p');
                excludeError.className = 'client-offer-card__error';
                excludeControl.appendChild(excludeError);
                excludeField.appendChild(excludeLabel);
                excludeField.appendChild(excludeControl);
                grid.appendChild(excludeField);
                ...
                offer.dom = {
                    ...
                    statusText,
                    errorElements: {
                        tipo_de_garantia: warrantyFieldObj.error,
                        nivel_garantia: levelFieldObj.error,
                        precio_fijo: priceFieldObj.error,
                        duracion_maxima: durationFieldObj.error,
                    },
                    fieldWrappers: {
                        tipo_de_garantia: warrantyFieldObj.field,
                        nivel_garantia: levelFieldObj.field,
                        precio_fijo: priceFieldObj.field,
                        duracion_maxima: durationFieldObj.field,
                    },
                };
                ...
                warrantySelect.addEventListener('change', (event) => {
                    ...
                    offer.tipo_de_garantia = {
                        value,
                        label: specialChoiceMaps.tipo.get(value) || '',
                    };
                    clearFieldError(offer, 'tipo_de_garantia');
                    updateOfferMetadata(offer, index);
                    updateDirtyState();
                });
                ...
                levelSelect.addEventListener('change', (event) => {
                    const value = Number(event.target.value);
                    offer.nivel_garantia = {
                        value,
                        label: specialChoiceMaps.nivel.get(value) || '',
                    };
                    clearFieldError(offer, 'nivel_garantia');
                    updateOfferMetadata(offer, index);
                    updateDirtyState();
                });

                priceInput.addEventListener('input', (event) => {
                    offer.precio_fijo = event.target.value;
                    clearFieldError(offer, 'precio_fijo');
                    updateDirtyState();
                });

                durationSelect.addEventListener('change', (event) => {
                    const value = Number(event.target.value);
                    offer.duracion_maxima = {
                        value,
                        label: specialChoiceMaps.duration.get(value) || '',
                    };
                    clearFieldError(offer, 'duracion_maxima');
                    updateDirtyState();
                });

                statusInput.addEventListener('change', (event) => {
                    offer.estado = Boolean(event.target.checked);
                    updateOfferMetadata(offer, index);
                    updateDirtyState();
                });

                excludeInput.addEventListener('change', (event) => {
                    offer.excluir_resto_de_niveles = Boolean(event.target.checked);
                    updateDirtyState();
                });
            ...
            function buildOfferCard(offer, index) {
                if (isFixedOffer(offer)) {
                    return buildFixedOfferCard(offer, index);
                }
                const card = document.createElement('article');
                card.className = 'client-offer-card';
                card.dataset.uid = offer.uid;
            ...
            function updateOfferMetadata(offer, index) {
                ...
                const typeLabel = isFixedOffer(offer)
                    ? (offersFixedPriceLabel || strings.offersFixedPrice || 'Precio fijo')
                    : (choiceMaps.tipo.get(offer.tipo_oferta) || (offer.tipo_oferta || ''));
                if (offer.dom.badge) {
                ...
                if (offer.dom.card) {
                    ...
                    offer.dom.card.classList.toggle(
                        'client-offer-card--sin-suplementos',
                        !isFixedOffer(offer) && offer.tipo_oferta === 'sin_suplementos',
                    );
                }
            }
            ...
            function duplicateOffer(index) {
                ...
                if (isFixedOffer(source)) {
                    clone.tipo_de_garantia = { ...source.tipo_de_garantia };
                    clone.nivel_garantia = { ...source.nivel_garantia };
                    clone.duracion_maxima = { ...source.duracion_maxima };
                } else {
                    clone.seleccion_modalidad = Array.isArray(source.seleccion_modalidad)
            ...
            function serializeOffers() {
                ...
                offers.forEach((offer) => {
                    if (isFixedOffer(offer)) {
                        const tipoValue = Number(offer?.tipo_de_garantia?.value);
                        const nivelValue = Number(offer?.nivel_garantia?.value);
                        const durationValue = Number(offer?.duracion_maxima?.value || offer?.duracion_maxima);
                        let priceValue = Number(offer?.precio_fijo);
                        if (!Number.isFinite(priceValue)) {
                            priceValue = '';
                        }
                        fixed.push({
                            tipo_de_garantia: Number.isFinite(tipoValue) ? tipoValue : 0,
                            nivel_garantia: Number.isFinite(nivelValue) ? nivelValue : 0,
                            precio_fijo: priceValue,
                            excluir_resto_de_niveles: Boolean(offer.excluir_resto_de_niveles),
                            duracion_maxima: Number.isFinite(durationValue) && durationValue > 0 ? durationValue : 12,
                    ...
                    standard.push(payload);
                });

                return { offers: standard, special_offers: fixed };
            }
            ...
            function validateOffers() {
                ...
                offers.forEach((offer) => {
                    offer.errors = offer.errors || {};
                    if (isFixedOffer(offer)) {
                        offer.errors.tipo_de_garantia = '';
                        offer.errors.nivel_garantia = '';
                        offer.errors.precio_fijo = '';
                        offer.errors.duracion_maxima = '';

                        const tipoValue = Number(offer?.tipo_de_garantia?.value);
                        const nivelValue = Number(offer?.nivel_garantia?.value);
                        const priceValue = Number(offer?.precio_fijo);
                        const durationValue = Number(offer?.duracion_maxima?.value || offer?.duracion_maxima);

                        if (!Number.isFinite(tipoValue) || tipoValue <= 0) {
                            offer.errors.tipo_de_garantia = strings.manageOffersFixedTypeError || 'Selecciona un tipo de garantía.';
                            isValid = false;
                        }

                        if (!Number.isFinite(nivelValue) || nivelValue <= 0) {
                            offer.errors.nivel_garantia = strings.manageOffersFixedLevelError || 'Selecciona una cobertura.';
                            isValid = false;
                        }

                        if (!Number.isFinite(priceValue) || priceValue <= 0) {
                            offer.errors.precio_fijo = strings.manageOffersFixedPriceError || 'Introduce un precio válido.';
                            isValid = false;
                        }

                        if (!Number.isFinite(durationValue) || durationValue <= 0) {
                            offer.errors.duracion_maxima = strings.manageOffersFixedDurationError || 'Selecciona una duración.';
                            isValid = false;
                        }
            ...
            async function loadOffers(userId) {
                ...
                try {
                    ...
                    offers = normalizeAllOffers(data?.offers || [], data?.special_offers || []);
                    renderOffers();
                    setInitialSnapshot();
                    setStatus('');
                } catch (error) {
            ...
            async function handleSave(event) {
                ...
                try {
                    const response = await fetch(`${restBase}go/v1/clientes/${userId}/offers`, {
                        ...
                        body: JSON.stringify(serializeOffers()),
                    });
                    if (!response.ok) {
                        throw new Error(`Request failed: ${response.status}`);
                    }
