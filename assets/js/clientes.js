    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function renderOffersList(offers) {
            ...
            const items = offers.map((offer) => {
                ...
                if (expires !== '') {
                    metaParts.push(expires);
                }
                const meta = metaParts.length > 0
                    ? `<span class="client-detail__chip-meta">${metaParts.map((value) => escapeHtml(value)).join(' · ')}</span>`
                    : '';

                return `<li class="client-detail__chip">${title}${discount}${meta}</li>`;
            });
        ...
        function createAssignDialog() {
            ...
            function renderResults(messageOverride = '') {
                ...
                const itemsHtml = results.map((commercial) => {
                    ...
                    return `
                        <button type="button" class="client-dialog__result${isSelected ? ' is-selected' : ''}" role="option" data-id="${id}" aria-selected="${isSelected ? 'true' : 'false'}">
                            ...
                            <span class="client-dialog__result-indicator" aria-hidden="true">✓</span>
                        </button>
                    `;
                }).join('');
                ...
                resultsContainer.querySelectorAll('.client-dialog__result').forEach((element) => {
                    element.addEventListener('click', () => {
                        ...
                        element.setAttribute('aria-selected', element.classList.contains('is-selected') ? 'true' : 'false');
                    });
                });
            }
            ...
            return {
                open(context = {}) {
                    ...
                    const placeholder = typeof context.company === 'string' && context.company.trim() !== ''
                        ? context.company.trim()
                        : (typeof context.name === 'string' && context.name.trim() !== ''
                            ? context.name.trim()
                            : (strings.client || 'este cliente'));
                ...
                if (initialSlug) {
                    ...
                    if (matchedId) {
                        ...
                        if (targetRow && targetItem) {
                            selectRow(targetRow, targetItem, { suppressHistory: true });
                            initialSlug = '';
                            window.requestAnimationFrame(() => {
