...
class ClientRestController
{
    ...
    public static function delete_user(WP_REST_Request $request): WP_REST_Response
    {
        ...
        /**
         * WordPress exige la capability `delete_user` en `wp_delete_user()`. Para roles a los que
         * permitimos gestionar clientes (director comercial, gestor de garantías) pero que no
         * tienen esa capability asignada, forzamos un pase explícito de `map_meta_cap` sólo para
         * el usuario objetivo de esta petición.
         */
        $temp_cap_filter = null;
        if ($can_manage_clients && ! current_user_can('delete_user', $user_id)) {
            $temp_cap_filter = static function (array $caps, string $cap, int $user_id_check, array $args) use ($user_id): array {
            ...
            add_filter('map_meta_cap', $temp_cap_filter, 10, 4);
        }
        ...
        if ($temp_cap_filter !== null) {
            remove_filter('map_meta_cap', $temp_cap_filter, 10);
        }
    ...
    private static function get_active_offers(int $user_id): array
    {
        ...
        $modalidad_options = self::get_modalidad_options();
        $modalidad_map     = [];
        foreach ($modalidad_options as $option) {
            if (isset($option['id']) && isset($option['title'])) {
                $modalidad_map[(int) $option['id']] = (string) $option['title'];
            }
        }
        ...
        foreach ($rows as $offer) {
            ...
            if (is_array($scope_raw)) {
                foreach ($scope_raw as $scope_entry) {
                    if (is_array($scope_entry)) {
                        $value = $scope_entry['value'] ?? $scope_entry['label'] ?? '';
                        if ($value !== '') {
                            $scope_values[] = sanitize_key((string) $value);
                        }
                    } else {
                        $value = (string) $scope_entry;
                        if ($value !== '') {
                            $scope_values[] = sanitize_key($value);
                        }
                    }
                }
            } elseif ($scope_raw !== '') {
                $scope_values[] = sanitize_key((string) $scope_raw);
            }
            ...
            if (! empty($offer['seleccion_modalidad']) && is_array($offer['seleccion_modalidad'])) {
                foreach ($offer['seleccion_modalidad'] as $modalidad_id) {
                    $modalidad_id = (int) $modalidad_id;
                    if ($modalidad_id > 0) {
                        $selection_ids[] = $modalidad_id;
                    }
                }
            }

            $selection_titles = [];
            if (! empty($selection_ids) && ! empty($modalidad_map)) {
                foreach ($selection_ids as $id) {
                    if (isset($modalidad_map[$id])) {
                        $selection_titles[] = $modalidad_map[$id];
                    }
                }
            }
            ...
            } elseif ($type_value === 'descuento_a_partir' && $threshold > 0) {
                $label     = sprintf('Descuento a partir de %d garantías', $threshold);
                $activated = self::count_active_guarantees_current_month($user_id);
                $remaining = max(0, $threshold - $activated);

                $meta = [
                    'activadas_mes'            => $activated,
                    'restantes_hasta_descuento' => $remaining,
                    'umbral'                    => $threshold,
                    'en_descuento'              => $remaining === 0,
                    'tipo_meta'                 => 'descuento_a_partir',
                ];
            }
