...
class AccountRestController
{
    ...
    public static function notify_sepa_activation(int $user_id, array $sepa_details = [], array $options = []): void
    {
        $signed_meta = SepaMandateService::get_document_meta($user_id, SepaMandateService::TYPE_SIGNED);

        self::notify_user_sepa_activated($user_id, $sepa_details, $signed_meta, $options);
        self::notify_admin_sepa_activated($user_id, $sepa_details, $signed_meta, $options);
    }
    ...
    private static function notify_admin_sepa_activated(int $user_id, array $sepa_details, array $signed_meta, array $options = []): void
    {
        ...
        $triggered_by_admin = ! empty($options['uploaded_by_admin']);
        $actor_name = '';
        $actor_email = '';

        if (! empty($options['actor_name'])) {
            $actor_name = sanitize_text_field((string) $options['actor_name']);
        }

        if (! empty($options['actor_email'])) {
            $actor_email = sanitize_email((string) $options['actor_email']);
        }
        ...
        $context = [
            ...
            'trigger'   => [
                'uploaded_by_admin' => $triggered_by_admin,
                'actor'             => [
                    'name'  => $actor_name,
                    'email' => $actor_email,
                ],
