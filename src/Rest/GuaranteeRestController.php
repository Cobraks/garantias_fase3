                        ...
                        if (is_array($v)) {
                            ...
                            if (!empty($v['listado_descuentos_recargos']) && is_array($v['listado_descuentos_recargos'])) {
                                ...
                                foreach ($v['listado_descuentos_recargos'] as $row) {
                                    $tipo_raw = $row['tipo'] ?? '';
                                    $tipo     = is_array($tipo_raw) ? sanitize_text_field($tipo_raw['value'] ?? '') : sanitize_text_field($tipo_raw);
                                    ...
                                    $tipo_label_map = [
                                        'base'      => __('Base', 'garantias-online-360vo'),
                                        'recargo'   => __('Recargo', 'garantias-online-360vo'),
                                        'descuento' => __('Descuento', 'garantias-online-360vo'),
                                        'iva'       => __('IVA', 'garantias-online-360vo'),
                                        'total'     => __('Total', 'garantias-online-360vo'),
                                        'oferta'    => __('Oferta', 'garantias-online-360vo'),
                                    ];
                                    $tipo_label = $tipo !== '' && isset($tipo_label_map[$tipo]) ? $tipo_label_map[$tipo] : '';

                                    $list[] = [
                                        'tipo'       => $tipo !== '' ? ['value' => $tipo, 'label' => $tipo_label ?: $tipo] : '',
            ...
            if (!empty($acf_gc_payload)) {
                if (function_exists('update_field')) {
                    if (isset($gc['descuentos_y_recargos']) && is_array($gc['descuentos_y_recargos'])) {
                        update_field('garantia_contratada_descuentos_y_recargos', $gc['descuentos_y_recargos'], $post_id);
