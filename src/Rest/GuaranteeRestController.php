...
class GuaranteeRestController
{
    ...
    private static function collect_year_summary(array $statuses): array
    {
        ...
        $previous_label = $previous_start->format('Y');
    ...
    private static function collect_global_summary(array $statuses): array
    {
        ...
        $previous = self::summarize_period($statuses, $previous_start, $previous_end);
        $previous_label = $now->modify('-1 year')->format('Y');
    ...
    private static function collect_month_summary(array $statuses): array
    {
        ...
        $previous = self::summarize_period($statuses, $previous_start, $previous_end);
        $previous_label = self::format_month_label($previous_start);
        if ($previous_label !== '' && function_exists('mb_strtolower')) {
            $previous_label = mb_strtolower($previous_label, 'UTF-8');
        } elseif ($previous_label !== '') {
            $previous_label = strtolower($previous_label);
        }
    ...
    ): array
    {
        ...
        if ($previous_value <= 0.0 && $current_value > 0.0 && $previous_period_label !== '') {
            return [
                'current'    => $current_value,
                'previous'   => $previous_value,
                'percentage' => 0.0,
                'formatted'  => sprintf(
                    __('Sin datos %s', 'garantias-online-360vo'),
