...
class GuaranteeLogger
{
    ...
    public static function log(int $user_id, int $guarantee_id, string $event_type, string $details = ''): void
    {
        if ($event_type === 'status_changed') {
            ...
            if ($new_status !== '' && in_array($new_status, ['pendiente_pago', 'pendiente_cobro', 'pending_payment', 'pending_cobro'], true)) {
                return;
            }
        }
    ...
    ): array
    {
        ...
        switch ($event_type) {
            ...
            case 'email_skipped':
                $context = array_merge(
                    $context,
                    self::parse_email_details($details, $vendor['vendor_name'] ?? '')
                );
                break;
     ...
     */
    private static function parse_email_details(string $details, string $vendor_name): array
    {
    ...
    private static function status_label(string $status): string
    {
        ...
        $map = [
            ...
            'pending_cobro'    => __('Pendiente de cobro', 'garantias-online-360vo'),
            'pendiente_pago'   => __('Pendiente de pago', 'garantias-online-360vo'),
            'pendiente_cobro'  => __('Pendiente de cobro', 'garantias-online-360vo'),
