...
class UserProfileResolver
{
    ...
    public static function build_from_user(WP_User $user): array
    {
        ...
        $personal_first_name = self::get_first_name($user);
        $personal_last_name  = self::get_last_name($user);
        $personal_full_name  = self::build_full_name($personal_first_name, $personal_last_name);
        if ($personal_full_name === '') {
            $personal_full_name = $personal_name;
        }
        ...
        return [
            ...
            'personal_full_name'   => $personal_full_name,
    ...
    public static function get_first_name($user): string
    {
        ...
        $value = get_user_meta($object->ID, 'first_name', true);

        return self::sanitize_text($value);
    }

    public static function get_last_name($user): string
    {
        $object = self::resolve_user($user);
        if (! $object instanceof WP_User) {
            return '';
        ...
        $value = get_user_meta($object->ID, 'last_name', true);

        return self::sanitize_text($value);
    }

    public static function get_full_name($user): string
    {
        $first = self::get_first_name($user);
        $last  = self::get_last_name($user);

        return self::build_full_name($first, $last);
    }
    ...
    public static function get_vendor_labels(int $user_id): array
    {
        ...
        $full_name  = self::build_full_name($first_name, $last_name);
        if ($full_name === '') {
            $full_name = $personal;
        }
    ...
    private static function build_full_name(string $first, string $last): string
    {
        $first = self::sanitize_text($first);
        $last  = self::sanitize_text($last);

        $parts = array_filter([$first, $last], static function ($part) {
            return $part !== '';
        });
