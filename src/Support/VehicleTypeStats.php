...
class VehicleTypeStats
{
    ...
    public static function get_counts(): array
    {
        ...
        $vehicle_type_meta_key = 'datos_vehiculo_tipo_vehiculo';
        $contract_state_meta_key = 'estado_garantia_estado_contratacion';

        // Contamos garantías por tipo_vehiculo guardado en meta (term_id).
        // Excluimos trash/auto-draft para no sesgar el ranking visual.
        $sql = $wpdb->prepare(
            "SELECT vehicle_type.meta_value AS term_id, COUNT(DISTINCT p.ID) AS total
             FROM {$posts_table} p
             INNER JOIN {$postmeta_table} vehicle_type
                ON p.ID = vehicle_type.post_id
               AND vehicle_type.meta_key = %s
             INNER JOIN {$postmeta_table} contract_state
                ON p.ID = contract_state.post_id
               AND contract_state.meta_key = %s
             WHERE p.post_type = %s
               AND p.post_status NOT IN ('trash', 'auto-draft')
               AND contract_state.meta_value = 'activada'
               AND vehicle_type.meta_value REGEXP '^[0-9]+$'
             GROUP BY vehicle_type.meta_value",
            $vehicle_type_meta_key,
            $contract_state_meta_key,
