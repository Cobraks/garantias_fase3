...
class EmailIngestCli
{
     ...
     *
     * [--debug]
     * : Muestra información de diagnóstico (asuntos que no casan patrón).
     *
     * ## EXAMPLES
     *
    ...
    public static function handle(array $args, array $assoc_args): void
    {
        ...
        foreach ($uids as $uid) {
            ...
            if ($plate === '') {
                if ($debug && $subject !== '' && count($unmatched_subjects) < 10) {
        ...
        if ($debug && ! empty($unmatched_subjects)) {
            \WP_CLI::log('Asuntos no reconocidos (muestra):');
            foreach ($unmatched_subjects as $unmatched_subject) {
                \WP_CLI::log('- ' . $unmatched_subject);
            }
            \WP_CLI::log('Sugerencia: ajusta GO360_MAIL_SUBJECT_REGEX para el formato real de esos asuntos.');
        }
    }
    ...
    private static function build_config(array $file_config): array
    {
        ...
        return [
            ...
            'subject_regex'      => trim($read('GO360_MAIL_SUBJECT_REGEX', '/Notificaci[oó]n\s*aver[ií]a[\s:\-#]*([0-9]{4}[A-Z]{3}|[A-Z]{1,3}[0-9]{3,4}[A-Z]{0,3}|[A-Z0-9-]{5,12})/iu')),
    ...
    private static function extract_plate(string $subject, array $config): string
    {
        ...
        if (preg_match('/\b([0-9]{4}[A-Z]{3}|[A-Z]{1,3}[0-9]{3,4}[A-Z]{0,3})\b/i', $subject, $matches)) {
            return strtoupper(sanitize_text_field((string) $matches[1]));
        }
