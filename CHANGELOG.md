...
## 6.4.20 - 2026-03-30
- Nuevo flujo de ingestión de averías por correo vía WP-CLI (`wp go360 mail-ingest`) leyendo IMAP con configuración por `.env` fuera de `httpdocs`.
- Nuevo evento interno `incident.email_received` registrado en Activity Log (dashboard) y conectado al sistema de notificaciones.
- Notificación integrada para bandeja actual: título “Nueva avería · fecha”, cuerpo con remitente y matrícula detectada por asunto.
- Destinatarios restringidos para este evento: solo `administrator` y `go_garantias`.

## 6.4.19 - 2026-03-30
- FIX Resumen mensual/anual (KPI importe): corregida la nota de comparativa previa en modo cobrado/con pendientes para que use el importe real del periodo anterior (evita mostrar `0,00€` por limpieza prematura de `amount_map`).
- Se mantiene el cálculo de tendencias y comparativas de conteo según el modo activo (cobrado vs con pendientes).
...
## 6.4.3 - 2026-09-26
- Resumen de garantías: ajuste de porcentajes para comparar correctamente con el periodo anterior.
